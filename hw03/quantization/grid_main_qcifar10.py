...
from model.resnet import QResNet18
import random
import numpy as np
import argparse
import os
...
def train_classifiar(train_loader, test_loader, num_epoch, model, args, grid_args):
    run_file = "num_bits:{}_opt:{}_epoch:{}_lr:{}_seed:{}_wd:{}_batch:{}_id_acc".format(
        args.num_bits,
        grid_args.optimizer,
        grid_args.num_epoch,
        grid_args.lr_rate,
...
def write_results(filename, num_bits, opt, num_epoch, lr, seed, weight_decay, num_batch_size, acc):

    file_exists = os.path.isfile(filename)

    info = {}
    fieldnames = []
    info["num_bits"], info["opt"], info["epoch"], info["lr"], \
    info["seed"], info["weight_decay"], info["batch_size"], info["ACC"] = \
        num_bits, opt, num_epoch, lr, seed, weight_decay, num_batch_size, acc
                ...
                for wd in args.weight_decay:
                    for opt in args.optimizer:
                        ...
                        if args.do_train:
                            ...
                            write_results(
                                args.num_bits, args.eval_results, opt, epoch, lr_rate, args.seed, wd, batch_size, acc
                            )
...
def main():
    ...
    parser.add_argument(
        "--eval_results",
        default="/home/s1_u1/projects/quantization/checkpoints/qresnet_cifar10_result.csv",
        type=str,
        required=False,
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--out_dir",
        default="/home/s1_u1/projects/ID-PCA/checkpoints/qresnet18.pt",
        type=str,
        required=False,
    )
    parser.add_argument(
        "--run_file",
        default="/home/s1_u1/runs/CIFAR10/qresnet18_grid",
        type=str,
        required=False,
    )
    parser.add_argument(
